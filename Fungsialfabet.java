...
public class Fungsialfabet
{
	public static void main( String[] args )
	{
		...
		e(0);					// ???? e( ???? );    // pass it the number of letters to display (9)
		f("g");					// ???? f( ???? );    // displays the word you pass it beginning with "F"
		System.out.print(g()); // ???? g( ???? );    // returns the word to be displayed
		h(2);					// ???? h( ???? );    // tell it how many times to display the word (1)
		
		System.out.println();

		i(100);											// ???? i( ???? );    // pass it any integer and it will display a word beginning with I 
		System.out.print(j(2));											// ???? j( ???? );    // returns a different word depending on what you pass it (1-3)
		k();
		l(true,true);											// ???? k( ???? );    // displays a word beginning with K
		m(true,false);											// ???? l( ???? );    // displays a different word depending on the two booleans you pass it
		n("jiji");											// ???? m( ???? );    // displays a different word depending on the two booleans you pass it
		o();											// ???? n( ???? );    // displays the word you pass it beginning with "N"
		System.out.print(p());													// ???? o( ???? );    // displays a word beginning with O and returns a useless value
		q();											// ???? p( ???? );    // returns the word to be displayed
													// ???? q( ???? );    // displays the word
		System.out.println();

		System.out.print(r(true));// ???? r( ???? );    // returns a different word depending on if you pass it 'true' or 'false'
		s(7);// ???? s( ???? );    // pass it the number of letters to display (6)
		t("haiji");// ???? t( ???? );    // displays the word you pass it beginning with "T"
